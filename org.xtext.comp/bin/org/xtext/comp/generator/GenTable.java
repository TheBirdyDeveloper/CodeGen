...
public class GenTable {
	...
	public String nomsToString(){
		...
		for(Map.Entry<Code, List<Instr>> entry : listCode3Adr.entrySet()){
	...
	public void parseFunDecl(){
		...
		while(ite.hasNext()) {
			...
			this.createInstr(nextCode,commands,null);
		}
	}
	
	public void createInstr(Code code,EList<Command> listCommands, List<Instr> code3Adr) {
		if(code3Adr==null)	
			code3Adr = new LinkedList<Instr>();
		
		this.parseCommands(listCommands, code3Adr);
			
		listCode3Adr.put(code, code3Adr);
	}
